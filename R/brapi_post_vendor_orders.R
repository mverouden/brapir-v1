...
#' @param requiredServiceInfo list; required: TRUE; A map of additional data required by the requested service.
...
#'
#' @references \href{https://app.swaggerhub.com/apis/PlantBreedingAPI/BrAPI/1.3#/Vendor/post_vendor_orders }{BrAPI SwaggerHub}
#'
#' @family brapi_1.3
#' @family Vendor
#'
...
brapi_post_vendor_orders <- function(con = NULL, clientId = '', numberOfSamples = 0, plates = '', requiredServiceInfo = list(), sampleType = '', serviceIds = '') {
  ...
  ## Set class of output
  class(out) <- c(class(out), "brapi_post_vendor_orders")
