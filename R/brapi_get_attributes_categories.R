...
#'
#' @details List all available attribute categories.
#'
#' @return data.frame
#'
...
#'
#' @export
brapi_get_attributes_categories <- function(con = NULL, page = 0, pageSize = 1000) {
  ...
  callurl <- brapi_GET_callURL(usedArgs = usedArgs,
                               callPath = "/attributes/categories",
                               reqArgs = "",
                               packageName = "BrAPI",
                               callVersion = 1.3)
  ...
  ## Set class of output
  class(out) <- c(class(out), "brapi_get_attributes_categories")
